...
class AssettoCorsaData(object):
        ...
        def start(self):
            ...
            if not self.physics:
                self.mmapPhysic = mmap.mmap(0, ctypes.sizeof(SPageFilePhysics), "acpmf_physics")
                self.mmapgraphics = mmap.mmap(0, ctypes.sizeof(SPageFileGraphic), "acpmf_graphics")
                self.mmapStatic = mmap.mmap(0, ctypes.sizeof(SPageFileStatic), "acpmf_static")
                self.physics = SPageFilePhysics.from_buffer(self.mmapPhysic)
                self.graphics = SPageFileGraphic.from_buffer(self.mmapgraphics)
                self.static = SPageFileStatic.from_buffer(self.mmapStatic)
            #self.mmapStatic = mmap.mmap(-1, XYZ, u"Local\\acpmf_static")

        def ctypes_to_python(self,value):

            if isinstance(value, ctypes.Structure):
                return {field: self.ctypes_to_python(getattr(value, field)) for field, _ in value._fields_}
            elif isinstance(value, ctypes.Array):
                return [self.ctypes_to_python(elem) for elem in value]
            else:
                return value
        def structure_to_dict(self,struct_instance):
            ...
            for field, _ in struct_instance._fields_:
                ...
                value = getattr(struct_instance, field)
                result[field] = self.ctypes_to_python(value)
            return result
        def getData(self):
            data_physics = self.structure_to_dict(self.physics)
            data_graphics = self.structure_to_dict(self.graphics)
            data_static = self.structure_to_dict(self.static)
            data = {**data_physics,**data_graphics,**data_static}
            return data
        ...
        def stop(self):
            ...
            if self.mmapPhysic:
                self.mmapPhysic.close()# variables having four sides have the format FL,FR,RL,RR
            if self.mmapStatic:
                self.mmapStatic.close()
            if self.mmapgraphics:
                self.mmapgraphics.close()

            self.mmapPhysic = None
            self.mmapStatic = None
            self.mmapgraphics = None
            self.physics = None
            self.static = None
            self.graphics = None
